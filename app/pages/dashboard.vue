<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <!-- Company Dashboard -->
    <div
      v-else-if="session.data?.user.userType === 'COMPANY'"
      class="space-y-8">
      <!-- Header -->
      ...
      <div v-else>
        ...
        <Tabs default-value="partnership-requests" class="space-y-6">
          <TabsList class="grid w-full grid-cols-3">
            <TabsTrigger value="partnership-requests"
              >Partnership Requests</TabsTrigger
            >
          ...
          <TabsContent value="partnership-requests" class="space-y-6">
            <div class="flex items-center justify-between">
              ...
              <Badge variant="secondary"
                >{{ pendingPartnershipRequests.length }} pending</Badge
              >
            </div>

            <div v-if="pendingPartnershipRequests.length" class="space-y-4">
              <Card
                v-for="request in pendingPartnershipRequests"
                :key="request.id">
                <CardContent class="p-6">
                  <div class="flex items-start justify-between mb-4">
                    <div class="flex-1">
                      ...
                      <div v-if="request.introduction" class="mb-4">
                        ...
                        <p
                          class="text-sm text-muted-foreground p-3 bg-muted/50 rounded-lg">
            ...
            <div v-else class="text-center py-12">
              ...
              <p class="text-muted-foreground">
                When recruiters express interest in your bounties, they'll
                appear here.
              </p>
            </div>
          </TabsContent>
          ...
          <TabsContent value="my-bounties" class="space-y-6">
            ...
            <div v-else class="text-center py-12">
              ...
              <p class="text-muted-foreground mb-6">
                Create your first bounty to start finding talented recruiters.
              </p>
              <NuxtLink to="/create">
          ...
          <TabsContent value="submissions" class="space-y-6">
            <div class="flex items-center justify-between">
              ...
              <Badge variant="secondary"
                >{{ allSubmissions.length }} total</Badge
              >
            </div>

            <div v-if="allSubmissions.length" class="space-y-4">
              <Card
                v-for="submission in allSubmissions.slice(0, 10)"
                :key="submission.id">
                <CardContent class="p-6">
            ...
            <div v-else class="text-center py-12">
              ...
              <p class="text-muted-foreground">
                Once recruiters start submitting candidates, they'll appear
                here.
              </p>
            </div>
          </TabsContent>
    ...
    <div
      ...
      <p class="text-muted-foreground mb-6">
        Recruiters should use the My Bounties page to manage their
        collaborations.
      </p>
    ...
    <Dialog v-model:open="showRejectDialogState">
      <DialogContent class="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Please provide a reason for rejecting this partnership request. This
        ...
        <DialogFooter class="flex gap-2">
          <Button variant="outline" @click="showRejectDialogState = false"
            >Cancel</Button
          >
          <Button
...
import { useCompanyBountiesQuery } from "@/composables/useCompanyBountiesQuery";
import { useCompanyPartnershipRequestsQuery } from "@/composables/useCompanyPartnershipRequestsQuery";
...
// Fetch company data
const { data: bounties, isPending: pending } = useCompanyBountiesQuery(enabled);

// Partnership requests data
const { data: partnershipRequests } =
  useCompanyPartnershipRequestsQuery(enabled);
...
const pendingPartnershipRequests = computed(
  () =>
    partnershipRequests.value?.filter((req: any) => req.status === "PENDING") ||
    []
);

const pendingPartnerships = computed(
  () => pendingPartnershipRequests.value.length
);

const activeRecruiters = computed(() => {
  if (!bounties.value) return 0;
  const uniqueRecruiters = new Set();
  bounties.value.forEach((bounty: any) => {
    bounty.collaborations?.forEach((collab: any) => {
...
const totalSubmissions = computed(() => {
  if (!bounties.value) return 0;
  return bounties.value.reduce(
    (sum: number, bounty: any) => sum + (bounty._count?.submissions || 0),
    0
  );
});
...
const allSubmissions = computed(() => {
  ...
  bounties.value.forEach((bounty: any) => {
    if (bounty.submissions) {
      submissions.push(
        ...bounty.submissions.map((sub: any) => ({ ...sub, bounty }))
      );
    }
  });
...
const approvePartnership = async (requestId: string) => {
  ...
  try {
    ...
    await queryClient.invalidateQueries({ queryKey: ["company-bounties"] });
  } catch (error) {
...
const rejectPartnership = async () => {
  ...
  try {
    await $fetch(
      `/api/company/partnership-requests/${
        (selectedRequest.value as any).id
      }/reject`,
      {
        method: "POST",
        body: {
