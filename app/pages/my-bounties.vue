<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div v-if="!user" class="text-center py-16">
      ...
      <p class="text-muted-foreground mb-6">You need to sign in to view your bounties.</p>
      <NuxtLink to="/sign-in">
    ...
    <!-- Non-Recruiter Check -->
    <div v-else-if="user.userType !== 'RECRUITER'" class="text-center py-16">
      <h1 class="text-2xl font-bold mb-4">Recruiter Access Only</h1>
      <p class="text-muted-foreground mb-6">This page is only accessible to recruiters.</p>
    ...
    <div v-else>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <Card>
          <CardContent class="p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-medium text-muted-foreground">Total Collaborations</p>
                <p class="text-2xl font-bold">{{ collaborations?.length || 0 }}</p>
              </div>
        ...
        <Card>
          <CardContent class="p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-medium text-muted-foreground">Unlocked</p>
                <p class="text-2xl font-bold">{{ unlockedCollaborations.length }}</p>
              </div>
          ...
          <CardContent class="p-6">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-bold">{{ pendingCollaborations.length }}</p>
              </div>
          ...
          <div class="flex items-start justify-between mb-4">
            <div class="flex-1">
              ...
              <div class="flex items-center gap-4 text-xs text-muted-foreground">
                ...
                <div v-if="collaboration.bounty.deadline" class="flex items-center gap-1">
                  <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                  </svg>
            ...
            <div class="text-right">
              ...
              <div class="text-xs text-muted-foreground">
                {{ collaboration.bounty.payoutType === 'CASH' ? 'Cash' : 'Percentage' }}
              </div>
            </div>
          </div>
          
          <!-- Submission Section (only if unlocked) -->
          <div v-if="collaboration.isUnlocked && collaboration.bounty.status === 'OPEN'" class="mt-6 pt-6 border-t">
            <div class="flex items-center justify-between mb-4">
              ...
              <Button 
                ...
                size="sm"
              >
                {{ showSubmissionForm === collaboration.id ? 'Cancel' : 'New Submission' }}
              </Button>
            </div>
            
            <!-- Submission Form -->
            <div v-if="showSubmissionForm === collaboration.id" class="space-y-4 p-4 bg-muted/50 rounded-lg">
              ...
              <div class="flex items-center gap-4">
                <Button 
                  ...
                  :disabled="isSubmitting || !submissionForm.candidateName || !submissionForm.file"
                >
            ...
            <div v-if="collaboration.submissions?.length" class="mt-4">
              ...
              <div class="space-y-2">
                ...
                >
                  <div>
                    <p class="text-sm font-medium">{{ submission.candidateName }}</p>
      ...
      <div v-else class="text-center py-16">
        ...
        <p class="text-muted-foreground mb-6">
          You haven't expressed interest in any bounties yet. Start exploring opportunities!
        </p>
        <NuxtLink to="/bounties">
          <Button>Browse Bounties</Button>
        </NuxtLink>
...
const { data: collaborations, pending, refresh } = await useFetch('/api/recruiter/collaborations', {
  server: false,
  default: () => []
})
...
const totalSubmissions = computed(() => 
  collaborations.value?.reduce((sum, c) => sum + (c.submissions?.length || 0), 0) || 0
)
...
const toggleSubmissionForm = (collaborationId: string) => {
  showSubmissionForm.value = showSubmissionForm.value === collaborationId ? null : collaborationId
...
const submitCandidate = async (collaborationId: string) => {
  ...
  try {
    ...
    formData.append('candidateEmail', submissionForm.value.candidateEmail || '')
...
if (process.client && user.value && user.value.userType !== 'RECRUITER') {
  await navigateTo('/bounties')
}
</script>